...
const PRODUCTS_URL = (catID) =>  `http://localhost:3000/api/categories/products/${catID}`;
const PRODUCT_INFO_URL = (id) => `http://localhost:3000/api/products/${id}`; 
const PRODUCT_INFO_COMMENTS_URL = (id) => `http://localhost:3000/api/products/comments/${id}`; 
const CART_INFO_URL = `http://localhost:3000/api/user/cart`; 
const CART_BUY_URL = `http://localhost:3000/api/cart`; 

// Obtener datos JSON usando getJSONData (función ya implementada en tu código)
getJSONData(CATEGORIES_URL).then((response) => {
    if (response.status === "ok") {
        console.log(response.data); // Maneja las categorías
    }
});

// Ejemplo de uso dinámico de URLs
const catID = "123"; // ID de categoría de ejemplo
getJSONData(PRODUCTS_URL(catID)).then((response) => {
    if (response.status === "ok") {
        console.log(response.data); // Maneja los productos de la categoría
    }
});

const productID = "45678"; // ID de producto de ejemplo
getJSONData(PRODUCT_INFO_URL(productID)).then((response) => {
    if (response.status === "ok") {
        console.log(response.data); // Maneja la información del producto
    }
});

getJSONData(PRODUCT_INFO_COMMENTS_URL(productID)).then((response) => {
    if (response.status === "ok") {
        console.log(response.data); // Maneja los comentarios del producto
    }
});
